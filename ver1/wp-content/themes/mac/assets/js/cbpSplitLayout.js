...
(function() {
	...
	/*rowcount = 1;
	$('#but').click(function() {	
			$('.ss').attr('id', 'test_'+rowcount);	
			$('.ss').append("<span classs='text'></span>");	
			$('.ss span').text(+rowcount);
	rowcount++;	
	});*/
	jQuery('.page_down').click(function(){
		var k = jQuery('.ms-section.active').attr('id');
		//alert(k)
	});
	
	var splitlayout = document.getElementById( 'splitlayout' ),
		leftSide = splitlayout.querySelector( 'div.intro > div.side-left' ),
		rightSide = splitlayout.querySelector( 'div.intro > a.side-right' ),
